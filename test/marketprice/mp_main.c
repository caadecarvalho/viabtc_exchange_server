...
int main(int argc, char *argv[])
{
    ...
    redisContext *redis = redisConnectWithTimeout("hci-redis-cluster.054ncr.ng.0001.use1.cache.amazonaws.com", 6379, timeout);
    if (redis == NULL || redis->err) {
        if (redis) {
            redisFree(redis);
        }
    }
    redisReply *reply = redisCommand(redis, "SENTINEL get-master-addr-by-name %s", "hcp-redis-cluster");
